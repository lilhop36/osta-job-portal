...
require_auth(null, app_base_path() . '/login.php');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_as_read'])) {
    if (!isset($_POST['csrf_token']) || !verify_csrf_token($_POST['csrf_token'])) {
        ...
        try {
            ...
            if (isset($_POST['notification_id'])) {
                $stmt = $pdo->prepare("UPDATE notifications SET status = 'read' 
                                      ...
                                      AND (target = 'all' 
                                          ...
                                          OR (target = 'department' AND target_id IN (SELECT department_id FROM users WHERE id = ?))
                                          OR (target = 'job' AND target_id IN (SELECT job_id FROM applications WHERE user_id = ?)))");
                $stmt->execute([$_POST['notification_id'], $_SESSION['user_id'], $_SESSION['user_id'], $_SESSION['user_id']]);
            ...
            $pdo->rollBack();
            $error = 'Error updating notification status. Please try again.';
...
$total_stmt = $pdo->prepare("SELECT COUNT(*) FROM notifications 
    WHERE (target = 'all' 
        ...
        OR (target = 'department' AND target_id IN (SELECT department_id FROM users WHERE id = ?))
        OR (target = 'job' AND target_id IN (SELECT job_id FROM applications WHERE user_id = ?)))");
$total_stmt->execute([$_SESSION['user_id'], $_SESSION['user_id'], $_SESSION['user_id']]);
...
$stmt = $pdo->prepare("SELECT n.*, u.username as creator_name 
                      ...
                      LIMIT ? OFFSET ?");
$stmt->execute([$_SESSION['user_id'], $_SESSION['user_id'], $_SESSION['user_id'], $per_page, $offset]);
