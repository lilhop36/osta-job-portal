...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['csrf_token']) && verify_csrf_token($_POST['csrf_token'])) {
    if (isset($_FILES['document']) && $_FILES['document']['error'] === UPLOAD_ERR_OK) {
        try {
            ...
            if (!DocumentService::isAllowedExtension($file['name'], $type_config['formats'])) {
            ...
            $stored_filename = DocumentService::buildStoredFilename($document_type, $file_extension);
...
if (isset($_POST['delete_document'], $_POST['csrf_token']) && verify_csrf_token($_POST['csrf_token'])) {
    try {
...
<head>
    ...
    <title><?php echo htmlspecialchars($page_title); ?> - OSTA Job Portal</title>
